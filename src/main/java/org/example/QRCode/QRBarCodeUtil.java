...
public class QRBarCodeUtil {
    ...
    public static void createCodeToFile(String codeContent, File codeImgFileSaveDir,String fileName) throws WriterException, IOException {
        try {
            if (codeContent == null || "".equals(codeContent.trim())) {
                System.out.println("二维码内容为空,无法操作!");
                return;
            }
            ...
            if (codeImgFileSaveDir == null || codeImgFileSaveDir.isFile()) {
                ...
                System.out.println("二维码图片存放目录为空,默认放置在桌面!");
            }

            if (!codeImgFileSaveDir.exists()) {
                codeImgFileSaveDir.mkdirs();
                System.out.println("二维码图片存放目录不存在,开始创建!");
            }

            if (fileName == null || "".equals(fileName)) {
                fileName = System.currentTimeMillis() + ".png";
                System.out.println("二维码图片文件名为空,使用当前时间命名");
            } else {
            ...
            boolean val = ImageIO.write(bufferedImage, "png", codeImgfile);
            if (val != false) {
